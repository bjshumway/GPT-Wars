...
async function makeGPTInternal(prompt, maxTok=4097, tempr = 0.7, frequenceyPenalty = 0.0,suffix) {
...
async function commentOnArchitype(architype) {
  ...
  $('#go-to-description-comment').disasble();
  ...
  $('#go-to-description-comment').enable();
...
//To save on space remove the chunks
storyLogic.chunks = undefined;

//Get rid of "too many new lines"
storyLogic.rawStoryText =  storyLogic.rawStoryText.replace(/\n{3,}/g,'\n\n');
      ...
      if(userObj.voiceAudioOn) {
        ...
        var part1 = storyLogic.rawStoryText.split('\n\n').slice(0                              , Math.round((numParagraphs/4)*1)).join('\n\n');
        var part2 = storyLogic.rawStoryText.split('\n\n').slice(Math.round((numParagraphs/4)*1), Math.round((numParagraphs/4)*2)).join('\n\n');
        var part3 = storyLogic.rawStoryText.split('\n\n').slice(Math.round((numParagraphs/4)*2), Math.round((numParagraphs/4)*3)).join('\n\n');
        var part4 = storyLogic.rawStoryText.split('\n\n').slice(Math.round((numParagraphs/4)*3), Math.round((numParagraphs/4)*4)).join('\n\n');
